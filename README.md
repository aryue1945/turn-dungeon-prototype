...
- Choose Basic Sword, Long Sword, or War Hammer before starting: Up/Down then Enter/Space, number keys 1/2/3, or mouse. The first button receives focus. War Hammer pushes a surviving hit target one cell back if the destination is walkable and unoccupied (a blocked push still deals damage). Each weapon shows a small pixel-art icon (`Art/Weapons/`) sized to its reach - 32x32 for Basic Sword/War Hammer's 1-cell reach, 64x32 for Long Sword's 2-cell reach - on its selection button and next to its name in the HUD.
