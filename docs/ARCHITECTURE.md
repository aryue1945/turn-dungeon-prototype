...
| `Scripts/Game/DebugHistoryExporter.cs` | `ToJson(DebugHistory)` serializes it with the shared `GameJsonOptions` (indented, camelCase, string enums) for the Export Debug History action. Pure string-building, unit tested against parsed JSON structure; writing the file is Main's job (`OnExportDebugHistoryPressed`, `System.IO.File.WriteAllText` under `OS.GetUserDataDir()`), reachable from a HUD button visible during play and on the end screen. |
...
| `Scripts/Game/RunSaveEnvelope.cs`, `RunSaveSerializer.cs` | Milestone-4 work: `RunSaveEnvelope` wraps a `GameSnapshot` with `SchemaVersion` and `Seed` (the "Configuration" bucket in docs/SAVE_AND_DEBUG_HISTORY.md - generation config kept separate from committed state). `RunSaveSerializer.ToJson`/`FromJson` round-trip it, rejecting an unsupported schema version or unparsable JSON with a typed `RunSaveLoadOutcome` instead of throwing into caller code. Unit tested for full round-trip fidelity (seed, turn number, status, every cell, actor fields including facing and attack preparation) and rejection paths. No file I/O yet - this is serialization only. |
...
| DebugHistory | Retain the last 10 turn transitions and starting state; export human/AI-readable JSON. Done: ring, wiring into Main, and the Export Debug History action/JSON writer. |
...
There are 98 test methods: generator/terrain 8, weapon 6, digging 4, dynamic terrain 4, mod loader 8, ActorState 14, GameState 8, TurnResolver 6, enemy movement behaviors 7, GameSnapshot 6, DebugHistory 9, DebugHistoryExporter 3, GameSnapshotRestore 5, actor restore 4, run save serialization 6. Source inspection only in this documentation review; no test execution claim.
