...
| `Scripts/Game/DebugHistory.cs` | `TurnTransition` bundles one consumed turn's direction, `PlayerActionOutcome`, `EnemyActionOutcome` list and resulting `GameSnapshot`. `DebugHistory` is the bounded ring: latest 10 transitions plus `BoundarySnapshot` (the snapshot immediately before the oldest retained transition - implemented as the evicted transition's own resulting snapshot). Engine-independent and unit tested, including the turn-25-retains-15..25 example from the spec. Not wired into Main yet - nothing constructs/appends to one during play, and there is no Export action. |
...
| DebugHistory | Retain the last 10 turn transitions and starting state; export human/AI-readable JSON. Ring implemented (`Scripts/Game/DebugHistory.cs`); export is still planned, and nothing in Main constructs or feeds one yet. |
