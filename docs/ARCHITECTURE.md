...
| `Scripts/Game/DebugHistoryExporter.cs` | `ToJson(DebugHistory)` serializes it with `System.Text.Json` (indented, camelCase, string enums) for the Export Debug History action. Pure string-building, unit tested against parsed JSON structure; writing the file is Main's job (`OnExportDebugHistoryPressed`, `System.IO.File.WriteAllText` under `OS.GetUserDataDir()`), reachable from a HUD button visible during play and on the end screen. |
