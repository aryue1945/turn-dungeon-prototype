...
| `Scripts/Game/GameSnapshotRestore.cs` | Milestone-4 work: `RestoreMap(GridSnapshot, seed)` is the inverse of `GameSnapshot.Capture`'s grid half - rebuilds a `DungeonMap` from saved terrain/durability/IsOpen/zone data instead of running fresh generation, via a new internal `DungeonMap.RestoreCell` primitive (`DungeonMap` itself stays unaware of `GridSnapshot`/`CellSnapshot`, keeping the Dungeon layer decoupled from the Game layer). Unit tested for cell-by-cell fidelity and independence from later mutation of the source map. Only the terrain half of restore; actor restoration, the save envelope, file I/O and Main's load flow do not exist yet. |
