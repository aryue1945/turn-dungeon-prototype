...
3. **First tactical gameplay slice (in progress).** War Hammer (knockback, done), Charging Beetle (telegraphed 2-cell charge), a cycling Spike Trap, and one designed fixed encounter combining all three. Implement each mechanic directly against the current turn/state model - no generic forced-movement/effect/hazard framework yet (see Architecture backlog: "Generalized effect or hazard frameworks").
