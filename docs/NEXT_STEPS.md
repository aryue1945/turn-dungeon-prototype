...
- 83 test methods covering generation, weapons, digging, disabled dynamic terrain, mod loading, ActorState, GameState, TurnResolver, enemy movement behaviors, GameSnapshot, DebugHistory, and its exporter.
- Milestone 1 (ActorState/GameState/grid combat) is done; milestone 2 (complete-turn execution) is substantially done; milestone 3 (snapshot capture/debug history) is substantially done - see below.
...
Remaining, deliberately deferred: `GameSnapshot`'s identity/config fields (schema version, run/floor id, generation request beyond seed, content fingerprints) stay deferred - still not meaningful with no run/floor concept, and milestone 4 (save/resume) is what will actually need most of them. The Main wiring (DebugHistory construction/append, the export button handler) has no automated test - Main is a Godot node - only the pieces it calls are unit tested.
