...
- 118 test methods covering generation, weapons, digging, disabled dynamic terrain, mod loading, ActorState, GameState, TurnResolver, enemy movement behaviors, GameSnapshot, DebugHistory, its exporter, map restore, actor restore, run save serialization, and the save-file service (backup rotation/recovery).
...
Remaining: the load/save flow in Main itself - showing Continue/New Run at startup when a resumable save exists, Continue restoring `GameState` and rebuilding views/camera/HUD from it instead of fresh generation (skipping weapon selection), New Run confirming before it overwrites an unfinished run, resolving saved WeaponId (`WeaponDefinitions`) and DefinitionId (`MonsterDefinitions`/mods) back into real definitions, and autosaving once per completed-turn boundary (including the turn that ends in victory/death, with no separate second save for that same turn). RNG continuation is out of scope per the acceptance note below.
