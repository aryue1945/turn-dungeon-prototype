...
abstraction question is deferred to Active roadmap step 8, which now records both axes and the
rejected shapes. Nothing else from that session needs carrying forward.

## Good next tasks for a smaller/cheaper model

Well-specified, localized, covered by existing tests.

1. **Step 6, weapon geometry as data.** `AttackDefinition` already carries `DetectionOffsets`,
...
2. **Step 5, momentum / multiplier.** Self-contained: a counter on run state, reset conditions, and
...
- **Step 8 itself.** It is a judgement call about five accumulated mechanics, not an
  implementation task.
...
wall-bump (used to stall enemy timing) a real cost. Pairs with step 5's momentum for the same
